...
test("compacts raw Notion blocks and removes noisy metadata", () => {
  ...
  assert.equal(result._gateway.version, "0.5.15");
...
test("publishes only the safety-scoped GPT Action operations with bounded page reads", () => {
  ...
  assert.equal(patched.info.version, "0.5.15");
  assert.equal(
...
test("publishes the minimal structured gameplay manifest for ChatGPT Actions", async () => {
  ...
  assert.equal(spec.info.version, "0.5.15");
  ...
  assert.ok(spec.paths["/world/load"]);
  ...
  const loadSchema = spec.paths["/world/load"].post.requestBody.content["application/json"].schema;
  assert.equal(loadSchema.properties.refresh.default, false);
  assert.deepEqual(loadSchema.properties.profile.enum, [
  ...
  assert.equal(updateSchema.anyOf.length, 2);
  assert.deepEqual(updateSchema.anyOf[1], { required: ["mutations"] });
  assert.ok(updateSchema.properties.mutations);
  assert.equal(updateSchema.properties.mutations.maxItems, 9);
  ...
  assert.deepEqual(updateSchema.properties.pageKey.enum, [
    "save", "character", "timeline", "knowledge", "relationships",
    "causality", "clues", "events", "director", "experience",
  ]);
  assert.equal(updateSchema.properties.pageId, undefined);
  assert.ok(updateSchema.properties.blockUpdates.items.required.includes("matchPrefix"));
  assert.equal(updateSchema.properties.blockUpdates.items.properties.blockId, undefined);
...
test("the Pages handler serves the filtered 0.5.15 OpenAPI document", async () => {
  const response = await onRequest({
  ...
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("X-Xuanche-Gateway-Version"), "0.5.15");
  assert.equal(document.info.version, "0.5.15");
  assert.equal(
...
test("rejects FAST_TURN_V1 loads and writes while the bound Worker is older than 0.5.17", async () => {
  let updateCalls = 0;
  const response = await onRequest({
  ...
  assert.match(body.error, /0\.5\.17/);
...
test("full Pages handler compacts a large module response before returning it to GPT", async () => {
  ...
  assert.equal(parsed._gateway.version, "0.5.15");
